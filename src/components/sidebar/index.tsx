...
const Sidebar = ({isMobileScreen}: IProps) => {
  ...
  return (
    <aside 
        ...
        className={`${(location.pathname == "/auth/signup" || location.pathname == "/user/profile" || location.pathname == "/auth/signin" || location.pathname == "/post-jobs" || location.pathname == "/community") ? "w-0 h-0" : "lg:w-[15%] md:w-[25%] w-[70%] h-screen p-[1.5rem]" } sidebar sticky inset-0 overflow-y-auto bg-(--white) flex-col ${isMobileScreen ? "md:hidden flex" : "md:flex hidden"} gap-5 border-r-[1px] md:z-0 z-[9] border-r-(--light-green)`}>
            ...
            <div className='w-[80%] flex flex-wrap gap-y-[.7rem] gap-x-[1.5rem]'>
                { positions.map(location => (
                    <div 
                        ...
                        <aside className={`bg-(--light-green) border-[1px] border-(--border-col) h-[15px] w-[15px] rounded-[5px] ${location.isActive ? "bg-(--light-green)" : "bg-transparent"}`}>
          ...
          <article className='flex w-full flex-col gap-[10px]'>
            ...
            <div className='w-[80%] flex flex-wrap gap-y-[.7rem] gap-x-[1.5rem]'>
                { types.map(type => (
                    <div 
                        onClick={() => {
                            handleFilterTabs(types, type.id, setTypes)
                        }}
                        key={type.id} className='flex items-center gap-[5px] cursor-pointer'>
                        <aside className={`bg-(--light-green) border-[1px] border-(--border-col) h-[15px] w-[15px] rounded-[5px] ${type.isActive ? "bg-(--light-green)" : "bg-transparent"}`}>
                            <Check className={`w-[clamp(.8rem,1vmax,1rem)] h-[clamp(.8rem,1vmax,1rem)] text-(--white) ${type.isActive ? "block" : "hidden"}`} />
                        </aside>
            ...
            <h4 className='font-semibold text-[clamp(.8rem,1.1vmax,1.1rem)]'>Salary Range</h4>
              <div className='w-[80%] flex flex-wrap gap-y-[.7rem] gap-x-[1.5rem]'>
                { salaryRanges.map(salary => (
                    <div 
                        onClick={() => {
                            handleFilterTabs(salaryRanges, salary.id, setSalaryRanges)
                        }}
                        key={salary.id} className='flex items-center gap-[5px] cursor-pointer'>
                        <aside className={`bg-(--light-green) border-[1px] border-(--border-col) h-[15px] w-[15px] rounded-[5px] ${salary.isActive ? "bg-(--light-green)" : "bg-transparent"}`}>
                            <Check className={`w-[clamp(.8rem,1vmax,1rem)] h-[clamp(.8rem,1vmax,1rem)] text-(--white) ${salary.isActive ? "block" : "hidden"}`} />
                        </aside>
                        <p className='text-[clamp(.8rem,1vmax,1rem)]'> {salary.title} </p>
                    </div>
                )) }
            </div>
         ...
         <article className='flex w-full flex-col gap-[10px]'>
            ...
            <div className='w-[50%] flex flex-wrap gap-y-[.3rem] gap-x-[1.1rem]'>
                { roles.map(role => (
                    <div 
                        onClick={() => {
                            handleFilterTabs(roles, role.id, setRoles)
                        }}
                        key={role.id} className='flex items-center gap-[5px] cursor-pointer'>
                        <aside className={`bg-(--light-green) border-[1px] border-(--border-col) h-[15px] w-[15px] rounded-[5px] ${role.isActive ? "bg-(--light-green)" : "bg-transparent"}`}>
                            <Check className={`w-[clamp(.8rem,1vmax,1rem)] h-[clamp(.8rem,1vmax,1rem)] text-(--white) ${role.isActive ? "block" : "hidden"}`} />
                        </aside>
                        <p className='text-[clamp(.8rem,1vmax,1rem)] whitespace-nowrap'> {role.title} </p>
                    </div>
                )) }
            </div>
        </article>
